...
import { MessageCircle, Mail, Github, Linkedin, ArrowRight, Send, CheckCircle, AlertCircle, Loader2 } from 'lucide-react'
...
const channels = [
  ...
  },
  {
    icon: <Mail size={26} />,
    title: 'E-mail',
    subtitle: 'gustavo@gustavodev.dev',
    description: 'Para projetos mais elaborados e propostas formais.',
    href: 'mailto:gustavo@gustavodev.dev',
    color: '#0ea5e9',
...
const socials = [
  ...
  { label: '@ogustavo_dev', href: 'https://instagram.com/ogustavo_dev', icon: (
  ...
  return (
    <section id="contato" ref={ref} className="py-16 lg:py-28 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        ...
        {/* Contact Form */}
        <motion.div
          variants={fadeUp} initial="hidden" animate={inView ? 'show' : 'hidden'} custom={6}
          className="glass-card rounded-2xl border border-white/[0.08] p-6 md:p-8 mb-10"
        >
          <h3 className="text-white font-bold text-lg mb-1">Enviar mensagem</h3>
          <p className="text-slate-400 text-sm mb-6">Preencha o formulário e eu respondo no menor tempo possível.</p>

          <form onSubmit={handleSubmit} noValidate className="flex flex-col gap-4">
            <div className="grid sm:grid-cols-2 gap-4">
              <div className="flex flex-col gap-1.5">
                <label htmlFor="contact-name" className="text-xs font-medium text-slate-400 uppercase tracking-wide">
                  Nome
                </label>
                <input
                  id="contact-name"
                  ...
                  className="bg-white/[0.04] border border-white/[0.1] rounded-xl px-4 py-3 text-sm text-white placeholder-slate-600 outline-none focus:border-brand-500/50 focus:bg-brand-500/[0.04] transition-all"
                />
              </div>
              <div className="flex flex-col gap-1.5">
                <label htmlFor="contact-email" className="text-xs font-medium text-slate-400 uppercase tracking-wide">
                  E-mail
                </label>
                <input
                  id="contact-email"
            ...
            <div className="flex flex-col gap-1.5">
              <label htmlFor="contact-message" className="text-xs font-medium text-slate-400 uppercase tracking-wide">
                Mensagem
              </label>
              <textarea
                id="contact-message"
                ...
                required
                rows={5}
                placeholder="Conte sobre o seu projeto, prazo, orçamento..."
                value={form.message}
                onChange={handleChange}
                className="bg-white/[0.04] border border-white/[0.1] rounded-xl px-4 py-3 text-sm text-white placeholder-slate-600 outline-none focus:border-brand-500/50 focus:bg-brand-500/[0.04] transition-all resize-none"
              />
            </div>
            ...
            <div className="flex items-center gap-4 flex-wrap">
              <button
                ...
                className="glow-btn inline-flex items-center gap-2 px-6 py-3 rounded-xl text-sm disabled:opacity-60 disabled:cursor-not-allowed"
              >
                {status === 'sending' ? (
              ...
              {status === 'success' && (
                ...
                >
                  <CheckCircle size={14} /> Mensagem recebida! Responderei em breve.
                </motion.p>
              )}
              {status === 'error' && (
                <motion.p
                ...
                >
                  ...
                  <button type="button" onClick={() => setStatus('idle')} className="underline ml-1">Tentar novamente</button>
                </motion.p>
              )}
            </div>
          </form>
